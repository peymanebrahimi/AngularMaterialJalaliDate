...
describe("GregorianMomentDateAdapter", () => {
  ...
  it("should get long month names in English", () => {
  ...
  it("should create Moment date", () => {
    expect(adapter.createDate(2017, 0, 1).format()).toEqual(
      moment({ y: 2017, M: 0, d: 1 }).format()
    );
  });
  ...
  it("should add years Gregorian", () => {
    expect(
      adapter
        .addCalendarYears(moment({ y: 2017, m: JAN, d: 1 }), 1)
        .format("YYYY-MM-DD")
    ).toEqual(moment({ y: 2018, m: JAN, d: 1 }).format("YYYY-MM-DD"));
    expect(
      adapter.addCalendarYears(moment({ y: 2018, m: JAN, d: 1 }), -1).format()
    ).toEqual(moment({ y: 2017, m: JAN, d: 1 }).format());
  });

  it("should respect leap years when adding years", () => {
    expect(
      adapter.addCalendarYears(moment([2015, JAN, 1]), 1).format()
  ...
  it("should add months", () => {
    ...
    expect(
      adapter.addCalendarMonths(moment([2017, MAR, 1]), -1).format()
    ).toEqual(moment([2017, FEB, 1]).format());
  });
  ...
  it("should add days", () => {
    expect(
      adapter.addCalendarDays(moment([2017, MAR, 21]), 1).format()
    ).toEqual(moment([2017, MAR, 22]).format());
    expect(
      adapter.addCalendarDays(moment([2017, MAR, 21]), -1).format()
    ).toEqual(moment([2017, MAR, 20]).format());
  });
